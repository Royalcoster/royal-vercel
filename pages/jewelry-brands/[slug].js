...
export async function getStaticProps(context) {
  ...
  const resNl = await fetch("https://wp.royalcoster.com/wp-json/wp/v2/collections?slug=" + params.slug + "&lang=" + context.locale);
          ...
          <div className="product-panel row">
            {(productsLoaded && product && product.length > 0) ? (
              product.map((item, index) => (
                <Link
                href= {"javascript:window.location.href='/" + ((locale == 'en') ? "" : "nl/") + ((item.tags.indexOf("variants") == -1) ? "jewelry" : "shop") + "/" + window.location.href.split("/")[window.location.href.split("/").length - 2] + "/" + window.location.href.split("/")[window.location.href.split("/").length - 1] + "/" + getFilterValue(item.title) + "-" + item.shopifyid + "';"}
                 key={index}
                >
