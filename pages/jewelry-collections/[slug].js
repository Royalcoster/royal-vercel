...
export async function getStaticProps({ params, preview = false, previewData }) {
  ...
  const resEn = await fetch("https://wp.royalcoster.com/wp-json/wp/v2/collections?slug=" + params.slug);
  const dataEn = await resEn.json();

  const resNl = await fetch("https://wp.royalcoster.com/wp-json/wp/v2/collections?slug=" + params.slug + "&lang=nl");
  const dataNl = await resNl.json();

  return {
    props: {
        dataEn: dataEn[0].acf || {},
        seoDataEn: dataEn[0].yoast_head_json || {},
        dataNl: dataNl[0].acf || {},
            ...
            {(productsLoaded && product && product.length > 0) ? (
              product.map((item, index) => (
                <Link
                  passHref={true}
                    href={{
                      pathname: "/" + ((item.tags.indexOf("variants") == -1) ? "jewelry" : "shop") + "/[master]/[collection]/[slug]",
                      query: {
