...
function fireShare(media, cid, artsearch) {
  ...
  if (media == "facebook") {
  //  alert("share.royalcoster.com/shareBlog.php?url=" + window.location.href + "&id=" + (new Date()).getTime() + "&folder=" + blogURL + "&slug=" + uu + "&scrape=yes")
    FB.ui({
        method: 'share',
...
export async function getStaticProps({ params, preview = false, previewData }) {
  ...
  const resEn = await fetch(blogURL + "?slug=" + params.slug);
  const dataEn = await resEn.json();

  const resNl = await fetch(blogURL + "?slug=" + params.slug + "&lang=nl");
  const dataNl = await resNl.json();

  return {
    props: {
        dataEn: (dataEn[0] !== undefined) ? dataEn[0].acf : {},
        seoDataEn: (dataEn[0] !== undefined) ? dataEn[0].yoast_head_json : {},
        dataNl: (dataNl[0] !== undefined) ? dataNl[0].acf : {},
        seoDataNl: (dataNl[0] !== undefined) ? dataNl[0].yoast_head_json : {},
    },
  };
}
export default Page;
...
function Page(props) {
  ...
  useEffect(() => {
    if (router.query.slug) {
      ...
      let url = locale === "nl" ? blogURL + "?slug=" + router.query.slug + "&lang=nl" : blogURL + "?slug=" + router.query.slug;
      if (insideArr.find((item) => item == router.query.slug)) {
        url = locale === "nl" ? insideURL + "?slug=" + router.query.slug + "&lang=nl" : insideURL + "?slug=" + router.query.slug;
      }
