...
export default function LandingPage() {
  ...
  const  mainPageUrl = process.env.NEXT_PUBLIC_WORDPRESS_URL + "/wp-json/wp/v2/page_templates?slug=wedding-consult&lang=" + locale;
                  ...
                  $("#contactForm").validate({
                    ...
                    submitHandler: function(form) {
                      ...
                      $.ajax({
                        ...
                        //  wUrl=https%3A%2F%2Froyalcoster.com%2Fwedding-consult&firstName=nenad&lastName=markovic&email=cobol1962%40gmail.com
                        //  &phoneNumber=38166366709&language=English&guest=2&visitDateWedding=04-07-2022&scheduledTime=16%20%3A%2000&moreInformation=message
                          var rsp = [
                            {name: "consult_type", value: "wedding_consult"},
                            {name: "date", value: $("#visitDateWedding").val()},
                            {name: "time", value: $("#scheduledTime").val()},
                            {name: "first_name", value: $("#firstName1").val()},
                            {name: "last_name", value: $("#lastName1").val()},
                             {name: "your_email", value: $("#email1").val()},
                             {name: "your_phone_number", value: $("#phoneNumber1").val()},
                             {name: "your_message", value: $("#message1").val().replace(/(?:\r\n|\r|\n)/g, '<br>')}
                          ];
                          var dll = {
                            event: "form_signup_wedding_consult",
                            formId: "012",
                            response: rsp
                          }
                          console.log(dll);
    ...
    <div className="collection-detail_page">
      ...
      <div style={{paddingBottom:30}} className="overview-section r-container row pt-5">
          ...
          <div className="col-md-6 book-date-panel mt-md-0 mt-5">
                <form id="contactForm"  action="https://costercatalog.com/api/index.php?request=contactSupport" method="post" className="row contact-form">
                    ...
                    <input  type="hidden" id="phoneNumber1" />
                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-4"><input
                    onChange = {(e) => {
                      $("#email1").val(e.target.value);
                    }}
                    type="email" name="email" id="email" className="form-control px-4 py-3" placeholder="Email" /></div>
                    <div className="col-lg-6 col-md-6 col-sm-12 mb-4"><input
                    onChange = {(e) => {
                      $("#phoneNumber1").val(e.target.value);
                    }}
                    type="number" name="phoneNumber" id="phoneNumber" className="form-control px-4 py-3" placeholder="Tel" /></div>
                    ...
                    <div className="col-12 mb-5"><textarea
                    onChange = {(e) => {
                      $("#message1").val(e.target.value);
                    }}
                    className="form-control px-4 py-3" rows="4" name="moreInformation" id="moreInformation" placeholder="More information" spellCheck="false"></textarea></div>
                </form>
      ...
      {shopifyTag && collectionHandle && (
        ...
        <div className="collection-section">
            ...
            <CollectionDetails  basicPath="collection" collectionHandle={collectionHandle} shopifyTag={shopifyTag} />
        </div>
      )}
