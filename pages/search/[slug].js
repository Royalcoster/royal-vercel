 ...
 const  openContact = (e) => {
  ...
  useEffect(() => {
    if (router.query.slug) {
      ...
      let shopifyid = document.location.href.split("-");
      ...
      fetch(getProductURL, {
        ...
        .then((data) => {
          ...
          setMainImage(data.image.src.replace(".jpg", "_300x.jpg"));
        ...
        <div className="confirm-section py-5 mb-5 row r-container" style={{visibility: "hidden"}}>
          ...
          <div className="show-setting col-md-6 col-12 p-0 pt-5 ps-5">
            <div className="title-panel">
              ...
              { productData.product_type.toString().toLowerCase() == "watches" && (
                  <h4 style={{
                    fontWeight: 400,
                    fontSize: 27
                      ...
                      >
                        {sizeList.length > 0 &&
                          ...
                          sizeList.map((item, index) => {
                            return (
                              <option
                              style={(tags.indexOf(" " + item.toString()) > -1) ? {fontWeight:"bold", color:'green'} : {}}
                              available={(tags.indexOf(" " + item.toString()) > -1) ? "Delivery within 3 days" : "Delivery in 6 weeks with. We will contact you about the fastest delivery"} value={index} key={index}>
                                {(index > 0) ? item : "Select"}
                              </option>
                            );
                          })}
