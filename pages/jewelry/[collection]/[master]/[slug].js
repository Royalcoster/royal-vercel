 ...
 const  openContact = (e) => {
  ...
  const addCart = (e) => {
    ...
    if ($("#selectSize").length > 0) {
        currentSize = $("#selectSize").find("option:selected").val();
        if (currentSize == "-1") {
          let variant = "error";
                    ...
                    <div className="select-box">
                      ...
                      >
                        {sizeList.length > 0 &&
                          ...
                          available={(rs.indexOf(item.toString()) > -1) ? "Delivery within 3 days" : "Delivery in 6 weeks with. We will contact you about the fastest delivery"}
                              value={(index > 0) ? item.toString() : "-1"} key={index}>
