...
const ReadingProgress = ({ target }) => {
  ...
  //  alert(locale)
    ...
    const scrollListener = () => {
      ...
      setReadingProgress(((windowScrollTop - restHeight) / totalHeight) * 100);blogURL
    };
...
export async function getStaticProps(context) {
  ...
  const res = await fetch(blogURL + "?slug=" + params.slug + "&lang=" + context.locale);
...
function Brief(props) {
  ...
  const [mainBlogLink, setMainBlogLink] = useState("");
 ...
 function getBlogs(url = blogURL + "?slug=" + router.query.slug + "&lang=" + locale, reload = true) {
   ...
   fetch(url, {
     ...
     .then((data) => {
       ...
       if (reload) {
         ...
         getBlogs(insideURL + "?slug=" + router.query.slug + "&lang=" + locale, false)
       } else {
         data = data[0];
 ...
 function fireShare(media, cid, artsearch) {
   ...
   if (media == "facebook") {
     FB.ui({
         method: 'share',
         href: "share.royalcoster.com/shareBlog.php?url=" + window.location.href + "&id=" + (new Date()).getTime() + "&folder=" + mainBlogLink.split("?")[0] + "&slug=" + uu + "&scrape=yes",
     }, function(response){});
     return
   }
  ...
  useEffect(() => {
    var ww = setInterval(function() {
      if ($("#facebookLink").length > 0 && $("#facebookLink1").length > 0) {
        ...
        clearInterval(ww);
          ...
          $("#facebookLink1").bind("click", function() {
            fireShare("facebook");
          })
        }
    }, 100)
    if (router.query.slug) {
      ...
      setFacebookLink("javascript(fireShare('facebook'))");

      setLinkdinLink(
        "https://www.linkedin.com/shareArticle?mini=true&url=" + escape("http://share.royalcoster.com/shareBlog.php?url=" + window.location.href + "&folder=" + mainBlogLink.split("?")[0] + "&slug=" + router.query.slug)
      );
      setTwitterLink("https://twitter.com/share?url=" + escape("http://share.royalcoster.com/shareBlog.php?url=" + window.location.href + "&folder=" + mainBlogLink.split("?")[0] + "&id=" + (new Date().getTime()) + "&slug=" + router.query.slug));
      setWhatsAppLink('https://api.whatsapp.com/send?text=I wanted to share this with you: ' + encodeURI(window.location.href));
      if (router.query.slug == "watch") {
            ...
            >
              {products.map((item, index) => {
                return (
                  ...
                  >
                    <a><div className="product-image round">
